"""
Midwicket Custom Exceptions

Comprehensive exception hierarchy for robust error handling and debugging.
"""

class MidwicketError(Exception):
    """Base exception for all Midwicket errors."""
    pass

# Data and Ingestion Errors
class DataError(MidwicketError):
...
class QueryError(MidwicketError):
...
class DatabaseConnectionError(MidwicketError):
...
# of OSError).  Code that imported `from midwicket.exceptions import
...
# Model and ML Errors
class ModelError(MidwicketError):
...
class SessionError(MidwicketError):
...
class PluginError(MidwicketError):
...
class LiveError(MidwicketError):
