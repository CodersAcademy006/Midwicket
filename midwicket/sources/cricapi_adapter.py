...
into the standard Midwicket adapter contract (``match_id``, ``format``, ``info``,
``events``, ``raw``) and includes automatic retry with exponential back-off for
transient HTTP errors (429 / 5xx).

Usage::

    from midwicket.sources.cricapi_adapter import CricAPIAdapter
...
Requirements:
    ...
    - ``requests`` package (included in Midwicket dependencies).
...
class CricAPIAdapter(BaseAdapter):
    """
    Adapter for CricAPI cricket data. Normalizes output, validates responses, and matches the Midwicket adapter contract.
    """
