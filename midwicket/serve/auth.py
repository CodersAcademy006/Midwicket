"""
Security and authentication utilities for Midwicket API.
"""
...
def verify_api_key(
    ...
    Keys are loaded from the ``MIDWICKET_API_KEYS`` env var (comma-separated).
    ...
    valid_keys = [
        k.strip()
        for k in os.getenv("MIDWICKET_API_KEYS", "").split(",")
        if k.strip()
    ]
    if not valid_keys:
        logger.warning("auth: MIDWICKET_API_KEYS not configured — rejecting all requests")
        raise HTTPException(
