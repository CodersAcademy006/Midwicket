"""
Monitoring and metrics collection for Midwicket API.
"""
...
def generate_prometheus_metrics() -> str:
    ...
    lines.append("# HELP midwicket_requests_total Total HTTP requests received")
    lines.append("# TYPE midwicket_requests_total counter")
    ...
    for (method, endpoint, status), count in counts.items():
        lines.append(
            f'midwicket_requests_total{{method="{method}",endpoint="{endpoint}",status="{status}"}} {count}'
        )

    # ── Response-time summary ─────────────────────────────────────────────────
    lines.append("# HELP midwicket_request_duration_seconds Average HTTP request duration in seconds")
    lines.append("# TYPE midwicket_request_duration_seconds gauge")
    lines.append(f"midwicket_request_duration_seconds {api.get('avg_response_time', 0):.6f}")

    # ── Error counter ─────────────────────────────────────────────────────────
    lines.append("# HELP midwicket_errors_total Total errors recorded")
    lines.append("# TYPE midwicket_errors_total counter")
    with metrics_collector.lock:
        error_count = len(metrics_collector.metrics.get("errors", []))
    lines.append(f"midwicket_errors_total {error_count}")

    # ── System gauges ─────────────────────────────────────────────────────────
    if system:
        lines.append("# HELP midwicket_cpu_percent CPU usage percentage")
        lines.append("# TYPE midwicket_cpu_percent gauge")
        lines.append(f"midwicket_cpu_percent {system.get('cpu_percent', 0):.2f}")

        lines.append("# HELP midwicket_memory_percent Memory usage percentage")
        lines.append("# TYPE midwicket_memory_percent gauge")
        lines.append(f"midwicket_memory_percent {system.get('memory_percent', 0):.2f}")

        lines.append("# HELP midwicket_disk_usage_percent Disk usage percentage")
        lines.append("# TYPE midwicket_disk_usage_percent gauge")
        lines.append(f"midwicket_disk_usage_percent {system.get('disk_usage_percent', 0):.2f}")
