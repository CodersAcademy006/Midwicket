"""
Rate limiting utilities for Midwicket API.
"""
...
def _build_rate_limiter() -> RateLimiter | RedisRateLimiter:
    ...
    try:
        ...
        logger.warning(
            "Invalid MIDWICKET_RATE_LIMIT_REQUESTS_PER_MINUTE=%r; using default 60",
            raw_rpm,
        )
...
def _is_trusted_proxy(host: str | None) -> bool:
    """Return True when *host* matches MIDWICKET_TRUSTED_PROXIES."""
    if not host:
        return False

    raw = os.getenv("MIDWICKET_TRUSTED_PROXIES", "").strip()
    if not raw:
        return False
    ...
    for item in raw.split(","):
        ...
        try:
            ...
            logger.warning("Ignoring invalid MIDWICKET_TRUSTED_PROXIES entry: %r", token)
            continue
