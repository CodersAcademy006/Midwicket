"""
Midwicket Plugin System: Extensible Architecture
...
class PluginManager:
    ...
    def _get_allowlist() -> List[str]:
        ...
        Reads MIDWICKET_PLUGIN_ALLOWLIST (comma-separated).  An empty list means
        ...
        """
        import os
        raw = os.getenv("MIDWICKET_PLUGIN_ALLOWLIST", "")
    ...
    def _validate_module_path(module_path: str, allowlist: List[str]) -> None:
        ...
        if not allowlist:
            raise ValueError(
                "Plugin loading is disabled: MIDWICKET_PLUGIN_ALLOWLIST is empty. "
                "Set it to a comma-separated list of allowed package prefixes to "
                "enable plugins (e.g. MIDWICKET_PLUGIN_ALLOWLIST=myplugin)."
            )
        if not any(module_path == pfx or module_path.startswith(pfx + ".") for pfx in allowlist):
            raise ValueError(
                ...
                f"MIDWICKET_PLUGIN_ALLOWLIST to permit it."
            )
    ...
    def discover_plugins(self) -> List[PluginSpec]:
        ...
        If MIDWICKET_PLUGINS is set but MIDWICKET_PLUGIN_ALLOWLIST is empty, logs
        a warning and returns an empty list instead of loading anything.
        """
        import os
        plugin_list = os.getenv("MIDWICKET_PLUGINS", "")
        if not plugin_list:
            return []
...
# Auto-load plugins on import only when MIDWICKET_PLUGINS is set AND we are
# not in production mode.  In production, operators must call load_all_plugins()
# explicitly after validating their configuration.
import os as _os
_env = _os.getenv("MIDWICKET_ENV", "development")
if _os.getenv("MIDWICKET_PLUGINS") and _env != "production":
    try:
        load_all_plugins()
