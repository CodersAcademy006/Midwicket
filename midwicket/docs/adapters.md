# Midwicket Data Source Adapters

Midwicket supports flexible data ingestion through the Adapter pattern, allowing you to integrate cricket data from various sources. This guide covers built-in adapters and how to create custom ones for your specific data sources.

## Overview

Data source adapters provide a consistent interface for ingesting cricket data from different sources. Midwicket ships with several adapters and makes it easy to create custom ones.
...
**Usage:**
```python
from midwicket.sources.adapters.retrosheet import RetrosheetAdapter

adapter = RetrosheetAdapter()
# Adapt baseball data to Midwicket schema
```
...
```python
from midwicket.sources.adapters.base import BaseAdapter
...
class BaseAdapter:
    ...
    def get_match_data(self, match_id: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing match data in Midwicket schema format
        """
        raise NotImplementedError
...
class MyCustomAdapter(BaseAdapter):
    ...
    def get_match_data(self, match_id: str) -> Dict[str, Any]:
        ...
        # Transform to Midwicket schema format
        return self._transform_to_midwicket_schema(raw_data)
    
    def get_competition_name(self) -> str:
        """Return competition name."""
        return "Custom Cricket League"
    
    def _transform_to_midwicket_schema(self, raw_data: Dict) -> Dict:
        """Transform API data to Midwicket schema."""
...
from midwicket.sources.adapters.registry import AdapterRegistry
...
Ensure your adapter transforms data to Midwicket's Schema V1 format:

```python
def _transform_to_midwicket_schema(self, raw_data: Dict) -> Dict:
    """Transform to Midwicket Schema V1."""
    return {
        "info": {
...
class MyCustomAdapter(BaseAdapter):
    def get_match_data(self, match_id: str) -> Dict[str, Any]:
        try:
            ...
            return self._transform_to_midwicket_schema(response.json())
...
2. **Schema Compliance**: Ensure data matches Midwicket Schema V1
...
class PrivateLeagueAdapter(BaseAdapter):
    ...
    def get_match_data(self, match_id):
        # Query database and transform to Midwicket schema
        pass
```
...
class CSVAdapter(BaseAdapter):
    ...
    def get_match_data(self, match_id):
        # Read CSV and transform to Midwicket schema
        pass
```

## Further Reading

- [Midwicket Schema Documentation](../../schema/)
- [Cricsheet Format Specification](https://cricsheet.org/format/)
- [Midwicket API Documentation](api.md)

---

For questions about creating custom adapters, please visit [GitHub Discussions](https://github.com/CodersAcademy006/Midwicket/discussions).
