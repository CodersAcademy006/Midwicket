...
def _load_initial_model() -> WinPredictor:
    ...
    Path mode is disabled when MIDWICKET_ENV=production to prevent arbitrary
    pickle deserialization in production deployments.  Use the default shipped
    model or rebuild from source data instead.
    """
    mode = os.environ.get("MIDWICKET_WIN_MODEL_MODE", "default").lower()
    if mode == "path":
        env = os.environ.get("MIDWICKET_ENV", "development").lower()
        if env == "production":
            import warnings
            warnings.warn(
                "MIDWICKET_WIN_MODEL_MODE=path is disabled in production "
                "(MIDWICKET_ENV=production). Falling back to the default model. "
                "Rebuild the model from source data to deploy a custom model.",
                RuntimeWarning,
                stacklevel=2,
            )
        else:
            model_path = os.environ.get("MIDWICKET_WIN_MODEL_PATH", "")
            if model_path:
                expected_sha256 = os.environ.get("MIDWICKET_WIN_MODEL_SHA256", "")
                if not expected_sha256:
                    raise RuntimeError(
                        "MIDWICKET_WIN_MODEL_SHA256 must be set when using "
                        "MIDWICKET_WIN_MODEL_PATH (required to verify model integrity)."
                    )
                import hashlib
