...
class StreamIngestor:
    ...
    def __init__(self, query_engine: QueryEngine, max_workers: int = 4):
        ...
        # listener, so it stays off unless MIDWICKET_WEBHOOK_ENABLED=true.
        self.webhook_server = None
        self.webhook_enabled = os.getenv("MIDWICKET_WEBHOOK_ENABLED", "false").strip().lower() == "true"
        self.webhook_host = os.getenv("MIDWICKET_WEBHOOK_HOST", "localhost").strip() or "localhost"
    ...
    def _start_webhook_server(self):
        """Start the webhook HTTP server (opt-in via MIDWICKET_WEBHOOK_ENABLED)."""
        if not self.webhook_enabled:
            logger.info(
                "Webhook server disabled; set MIDWICKET_WEBHOOK_ENABLED=true to bind %s:%s.",
                self.webhook_host,
                self.webhook_port,
            )
            return
