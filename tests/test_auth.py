"""
Tests for midwicket.serve.auth — API key verification, JWT, and password hashing.
...
def reset_env(monkeypatch):
    ...
    monkeypatch.delenv("MIDWICKET_API_KEYS", raising=False)
    yield
...
class TestGenerateApiKey:
    def test_returns_url_safe_string(self):
        from midwicket.serve.auth import generate_api_key
...
class TestPasswordHashing:
    def test_hash_and_verify_roundtrip(self):
        ...
        try:
            from midwicket.serve.auth import hash_password, verify_password, HAS_PASSLIB
        except RuntimeError:
...
class TestJWT:
    def test_create_and_decode_roundtrip(self, monkeypatch):
        ...
        try:
            from midwicket.serve.auth import create_access_token, decode_access_token, HAS_JWT
        except ImportError:
            pytest.skip("python-jose not installed")
        if not HAS_JWT:
            pytest.skip("python-jose not installed")

        monkeypatch.setenv("MIDWICKET_SECRET_KEY", "jwt-test-secret-key")
